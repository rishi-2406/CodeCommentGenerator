...
# Globally ignore SyntaxWarnings that arise during ast.parse of legacy Python source code
...
def _clean_docstring(raw: str) -> str:
    ...
    # Drop noisy prefixes often present in autogenerated docs.
    first_para = re.sub(r"^(note|todo|warning|deprecated)\s*:\s*", "", first_para, flags=re.I)
    first_para = first_para.rstrip(".").strip() + "."
    return first_para if len(first_para) > 12 else ""


def _is_target_informative(target: str) -> bool:
    """Reject very generic or malformed targets to improve training quality."""
    if not target:
        return False
    t = target.strip()
    ...
    generic = {
        "function", "method", "object", "value", "data", "returns", "return",
        "this", "that", "given", "process", "handle",
    }
    informative_words = [w for w in words if w not in generic]
    if len(set(informative_words)) < 2:
        return False

    # Skip obvious stubs/placeholder docs
    if re.search(r"\b(todo|fixme|tbd|placeholder)\b", t, flags=re.I):
        return False
    return True


def _dedup_key(func_name: str, input_text: str, target_text: str) -> str:
    """Stable dedup key including AST-input and normalized target semantics."""
    normalized_target = re.sub(r"\W+", " ", target_text.lower()).strip()
    normalized_input = re.sub(r"\s+", " ", input_text.strip()).lower()
    digest = hashlib.md5(normalized_input.encode("utf-8")).hexdigest()[:10]
    return f"{func_name.lower()}::{normalized_target}::{digest}"
...
def _extract_pairs_from_source(
    ...
    for ff in mf.functions:
        ...
        if len(target) < min_doc_len or not _is_target_informative(target):
            continue
...
def build_codesearchnet_dataset(
    ...
    for row in ds:
        ...
        if len(target) < 12 or not _is_target_informative(target):
            continue

        # Deduplication key
        key = (func_name, re.sub(r"\W+", " ", target.lower()).strip()[:100])
        if key in seen:
            continue
        seen.add(key)
...
def build_full_dataset(
    ...
    if deduplicate:
        ...
        for p in pairs:
            key = _dedup_key(p.func_name, p.input_text, p.target_text)
            if key not in seen:
                seen.add(key)
                unique.append(p)
        pairs = unique
